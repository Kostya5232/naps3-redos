...
Карта текущего исходного кода версии **0.7** с исправлением выбора устройства и защитой сохранения, обновлена 8 сентября 2026 года. Источники: граф codebase-memory-mcp, реализации функций, тесты, установочные скрипты и конфигурация RPM.

NAPS3 — настольное приложение для сканирования документов на РЕД ОС 8. Пользователь подключает USB- или сетевой сканер, получает страницы со стекла или автоподатчика, меняет их порядок и ориентацию, затем сохраняет PDF, TIFF или отдельные файлы. Изображения и PDF также можно импортировать.

Основной код сосредоточен в **одном файле [naps3.py](naps3.py)** — около 6 130 строк. В нём 10 классов; главный класс `MainWindow` занимает около 3 897 строк и объединяет интерфейс, управление операциями и обработку страниц. Блоки ниже — логические части этого файла, а не отдельные Python-модули.
...
**Запуск.** Ярлык вызывает команду `naps3`, оболочка запускает системный Python. `main()` проверяет окружение и запускает `Naps3Application`; `do_activate()` создаёт `MainWindow`. Окно создаёт каталог сессии, читает настройки и строит интерфейс. Фоновая проверка обращается только к сохранённому адресу или SANE-устройству. Недоступный профиль сохраняется с сообщением об ошибке; другое МФУ автоматически не выбирается. Запоздалый результат проверки игнорируется после смены профиля или во время операции. При отсутствии профиля выполняется первоначальный поиск устройства, начиная с USB.

**Выбор устройства.** Профиль — словарь с именем, eSCL URL, SANE `device_id`, типом подключения, транспортом и возможностями АПД. Для USB сначала ищется локальный eSCL от ipp-usb. Если его нет, проверяется `hpaio`; устройство принимается только после успешного пробного открытия. Сетевой сканер можно выбрать из результатов поиска или подключить по адресу.
...
| Условие | Путь |
| --- | --- |
| Стекло | `_scan_once()` → `scanimage --format=png`. |
| USB-АПД, включая локальный ipp-usb | `_scan_once()` → `scanimage --batch` → PNM → PNG. |
...
В SANE-пакете используются PNM и буфер 1024 КБ; преобразование в PNG начинается после получения пакета и выполняется максимум в трёх потоках. Локальный USB намеренно остаётся на SANE: комментарии к коду фиксируют потерю страниц M428/M429 при прямом чтении eSCL через localhost.

В сетевом eSCL состояние связывается с URI текущего задания. Пустой АПД сам по себе не завершает получение: код учитывает ещё не переданные изображения. Полученная страница сначала пишется в `.part`, затем переименовывается. Завершённое задание освобождается через DELETE. При некоторых ошибках возможен переход к SANE; после HTTP 409/423/503 и длительного отсутствия следующей страницы автоматический переход блокируется.
...
| HPLIP / hpaio | Дополнительный USB-путь для совместимых моделей; ручной установщик устанавливает его при наличии пакета. |
| poppler-utils / `pdftoppm` | Импорт страниц PDF в изображения. |
| Bash, DNF/RPM, systemd | Установка, сборка пакета и обслуживание системного USB-транспорта. |
...
Реализация внутри бинарника `ipp-usb-naps3` не исследована: исходники отсутствуют. Его роль установлена по конфигурации, скриптам и комментариям проекта. Выпуск 0.7 включает этот ранее использовавшийся бинарник без изменений.
...
После доработок в Windows на Python 3.13 пройдены 63 проверки логики: 30 тестов защиты сохранения, 20 тестов восстановления устройства и 13 существующих тестов. Импорты GTK заменены заглушками, обращения к сканерам замоканы; операции Pillow и запись временных файлов выполнялись реально. Проверены синтаксис Python и ограниченный объём изменений по структуре исходника. Полный набор Linux-тестов, настоящее окно GTK и физическое сканирование здесь не проверялись. Команды и границы проверки приведены в [отчёте о защите сохранения](diagnostics/save-safety/README.md).
