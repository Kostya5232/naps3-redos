...
Карта текущего исходного кода версии **0.8.2** с поддержкой РЕД ОС и Windows, обновлена 9 сентября 2026 года. Источники: граф codebase-memory-mcp, реализации функций, тесты, установочные скрипты и конфигурации пакетов.
...
flowchart LR
    ...
    Tag["Тег v0.8.2"] --> Release["release.yml"]
...
Реализация внутри бинарника `ipp-usb-naps3` не исследована: исходники отсутствуют. Его роль установлена по конфигурации, скриптам и комментариям проекта. Версия 0.8.2 включает этот ранее использовавшийся Linux-бинарник без изменений.
...
Для версии 0.8.2 сохранены 84 модульные проверки: защита сохранения, восстановление выбранного устройства, Windows WIA, общий SANE-поиск и отдельный сценарий Canon MF4410 через `pixma`. Исправление 0.8.2 меняет только обнаружение установленного `pixma`, диагностику и установочные права USB; получение и обработка страниц не менялись. Обращения к физическим сканерам в тестах заменены управляемыми ответами, поэтому окончательная проверка подачи бумаги остаётся аппаратной.
