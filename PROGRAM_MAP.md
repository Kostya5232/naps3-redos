...
Карта текущего исходного кода версии **0.9.5** с TWAIN-подключением для Windows; обновлена 25 сентября 2026 года. Источники: граф codebase-memory-mcp, реализации функций, тесты, установочные скрипты и конфигурации пакетов.
...
Основной интерфейс и сценарии сосредоточены в [naps3.py](naps3.py). Граница Windows вынесена в [windows_backend.py](windows_backend.py), COM-вызовы WIA — в [windows/wia_bridge.cs](windows/wia_bridge.cs), а TWAIN — в отдельный 32-битный помощник [windows/twain_bridge.cs](windows/twain_bridge.cs). Главный класс `MainWindow` объединяет интерфейс, управление операциями и обработку страниц.
...
flowchart TD
    ...
    Scan --> Twain["_scan_windows_twain → Windows TWAIN"]
    ...
    Twain --> TwainDriver["Нативный TWAIN-драйвер"]
...
| [windows/twain_bridge.cs](windows/twain_bridge.cs), [windows/build_twain_bridge.py](windows/build_twain_bridge.py) | Изолированный 32-битный TWAIN-процесс с точным выбором источника, пределом страниц и проверкой поддерживаемого DPI. |
| [windows/vendor/NTwain.dll](windows/vendor/NTwain.dll) | Закреплённая версия NTwain 3.7.6 (MIT); при сборке проверяется SHA-256, лицензия поставляется рядом. |
...
| [windows/pin_cairo.py](windows/pin_cairo.py) | Подмена нестабильной DLL Cairo 1.18.6 на проверенную 1.18.4-4 из официального архива MSYS2 с проверкой SHA-256 при сборке Windows. |
| [windows/check_gui_startup.py](windows/check_gui_startup.py) | Проверка собранного EXE: запуск с отдельными настройками и ожидание видимого окна NAPS3. |
...
| [tests/test_scanner_recovery.py](tests/test_scanner_recovery.py) | 27 тестов сохранения выбранного устройства, миграции старого профиля HP, фоновой проверки при запуске и защиты от сканирования не тем МФУ. |
...
| Windows: выбранный TWAIN-профиль | `_scan_windows_twain()` → 32-битный `NAPS3.TwainBridge.exe` → нативный TWAIN → BMP → PNG; предел 40 страниц и контроль зависания. |

Для проверенного Kyocera TWAIN корректно передаёт изображения через USB, но `CAP_PAPERDETECTABLE` возвращает `False`, а `CAP_FEEDERLOADED` возвращает `False` даже при вложенном листе. Пустой АПД способен вернуть страницу со стекла. Поэтому TWAIN-скан из АПД начинается с подтверждения пользователем наличия бумаги; завершённый скан следует сверять с физическим прохождением листов.
...
Реализация внутри бинарника `ipp-usb-naps3` не исследована: исходники отсутствуют. Его роль установлена по конфигурации, скриптам и комментариям проекта. Версия 0.9.5 включает этот ранее использовавшийся Linux-бинарник без изменений.
...
Для версии 0.9.3 подготовлены 105 модульные проверки: защита сохранения, восстановление выбранного устройства, защита от сканирования сохранённым HP при поиске Kyocera, неблокирующий запуск Windows, строгий self-test, Windows WIA, DNS-SD/mDNS-поиск eSCL, сохранение объявленного порта и отсутствие повторного поиска, общий SANE-поиск, Canon MF4410 через `pixma`, атомарная подготовка изображения, отбрасывание оборванного PNM, однократный повтор занятого SANE-устройства и безопасная регистрация из Printer Doctor. Нативный WIA-помощник компилируется и проверяет выбор стекла при обратном порядке источников в своём `selftest`. После локального исправления сборка Windows также проверяет появление настоящего GTK-окна. Аппаратная проверка Kyocera ECOSYS MA4000x по USB выявила отдельную проблему: официальный подписанный WIA-драйвер 2.4.1123 зарегистрировал сканер, но операция `Connect` завершалась с `0x80210001` даже после пробуждения МФУ и перезагрузки ПК. Страница не получена; для сохранения исходной конфигурации возвращён прежний USB-драйвер `kmusbscn.inf`.
