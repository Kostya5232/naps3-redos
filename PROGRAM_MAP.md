...
Карта текущего исходного кода версии **0.8.1** с поддержкой РЕД ОС и Windows, обновлена 9 сентября 2026 года. Источники: граф codebase-memory-mcp, реализации функций, тесты, установочные скрипты и конфигурации пакетов.
...
| [packaging/60-naps3-scanners.rules](packaging/60-naps3-scanners.rules) | Доступ активного пользователя к Canon MF4410 с USB ID `04a9:2737`. |
| [build_rpm.sh](build_rpm.sh) | Сборка RPM для x86_64, размещение результата и контрольной суммы в `dist/`. |
| [.github/workflows/checks.yml](.github/workflows/checks.yml) | Проверка синтаксиса, 84 модульных тестов, GTK smoke-тест и проверка shell-скриптов при каждом обновлении `main`. |
...
flowchart LR
    ...
    Tag["Тег v0.8.1"] --> Release["release.yml"]
...
**Запуск.** В РЕД ОС ярлык вызывает команду `naps3`; в Windows установщик создаёт ярлыки для `NAPS3.exe`. `main()` проверяет платформенные компоненты и запускает `Naps3Application`; `do_activate()` создаёт `MainWindow`. Окно создаёт каталог сессии, читает настройки и строит интерфейс. Фоновая проверка обращается только к сохранённому адресу, SANE-устройству или точному WIA DeviceID. Недоступный выбранный профиль сохраняется с сообщением об ошибке; старый автоматически созданный профиль `MFP-YUR` удаляется, если HP больше не найден. Другое МФУ автоматически не выбирается.
...
Реализация внутри бинарника `ipp-usb-naps3` не исследована: исходники отсутствуют. Его роль установлена по конфигурации, скриптам и комментариям проекта. Выпуск 0.8.1 включает этот ранее использовавшийся Linux-бинарник без изменений.
...
Для версии 0.8.1 предусмотрены 84 модульные проверки: защита сохранения, восстановление выбранного устройства, Windows WIA, общий SANE-поиск и отдельный сценарий Canon MF4410 через `pixma`. GitHub Actions выполняет их на Linux и Windows; Linux дополнительно запускает настоящее окно GTK через Xvfb, а Windows — самопроверку собранного EXE. Обращения к физическим сканерам в тестах заменены управляемыми ответами, поэтому окончательная проверка подачи бумаги остаётся аппаратной.
