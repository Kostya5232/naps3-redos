...
Карта текущего исходного кода версии **0.8** с поддержкой РЕД ОС и Windows, обновлена 8 сентября 2026 года. Источники: граф codebase-memory-mcp, реализации функций, тесты, установочные скрипты и конфигурации пакетов.
...
Основной интерфейс и сценарии сосредоточены в [naps3.py](naps3.py). Граница Windows вынесена в [windows_backend.py](windows_backend.py), а COM-вызовы WIA — в [windows/wia_bridge.ps1](windows/wia_bridge.ps1). Главный класс `MainWindow` объединяет интерфейс, управление операциями и обработку страниц.
...
| [windows/wia_bridge.ps1](windows/wia_bridge.ps1) | Точный выбор WIA DeviceID, стекло/АПД/дуплекс и получение BMP через WIA COM. |
...
flowchart LR
    ...
    Tag["Тег v0.8"] --> Release["release.yml"]
...
**Запуск.** В РЕД ОС ярлык вызывает команду `naps3`; в Windows установщик создаёт ярлыки для `NAPS3.exe`. `main()` проверяет платформенные компоненты и запускает `Naps3Application`; `do_activate()` создаёт `MainWindow`. Окно создаёт каталог сессии, читает настройки и строит интерфейс. Фоновая проверка обращается только к сохранённому адресу, SANE-устройству или точному WIA DeviceID. Недоступный профиль сохраняется с сообщением об ошибке; другое МФУ автоматически не выбирается.

**Выбор устройства.** Профиль — словарь с именем, URL или `device_id`, типом подключения, транспортом и возможностями АПД. В РЕД ОС для USB сначала ищется локальный eSCL от ipp-usb, затем `hpaio`. В Windows быстро перечисляются зарегистрированные WIA-сканеры без подключения к каждому из них: это не задерживает список, когда МФУ спит. Сетевой eSCL-сканер можно подключить по адресу на обеих платформах.
...
| Условие | Путь |
| --- | --- |
| Windows: стекло или АПД | `_scan_windows_wia()` → PowerShell-мост → WIA COM → BMP → PNG. |
...
| Windows PowerShell 5.1 | Запуск поставляемого WIA COM-моста. |
...
Реализация внутри бинарника `ipp-usb-naps3` не исследована: исходники отсутствуют. Его роль установлена по конфигурации, скриптам и комментариям проекта. Выпуск 0.8 включает этот ранее использовавшийся Linux-бинарник без изменений.
